...
def onleftclick(rectangle, x, y):
    ...
    if data[y][x] == 8:
        ...
        #time.sleep(2)
        #root.destroy()
        '''SLIGHT PROBLEM WITH BACKEND TERMINATION?'''
...
def settingsMenu():
    global root
    global firstTimeSettings
    
    root.withdraw()
    rootSettings.deiconify()
    
      
    if firstTimeSettings == 0:
        ...
        settingsScreen.pack()
        firstTimeSettings = firstTimeSettings + 1
...
go = Button(root, text = "Start Game", height = 1, width = 12, font = ("courier new", '60'), command = gameStart)
...
#creating a canvas
gameArea = Canvas(root, height = 1000, width = 1000, bg = "tan")
